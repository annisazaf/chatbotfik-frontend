...
interface RiwayatModalProps {
  onClose?: () => void;
  onMulaiChat?: () => void;
  onLanjutChat?: (sessionId: string) => void; // klik item riwayat → lanjut sesi lama
}
...
export default function RiwayatModal({ onClose, onMulaiChat, onLanjutChat }: RiwayatModalProps) {
  ...
  useEffect(() => {
    fetch("${import.meta.env.VITE_API_URL || "http://localhost:5000/api"}/chatbot/sessions", { credentials: "include" })
      ...
      .then((d) => {
        ...
        // Setiap session perlu kita tambahkan 'preview' dari title atau message_count
  ...
  const handleSearch = (val: string) => {
    setSearch(val);
    setFiltered(
      items.filter((i) => i.title.toLowerCase().includes(val.toLowerCase()))
    );
  };
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-end" onClick={onClose}>
      ...
      >
        ...
        <div className="px-5 pb-6 pt-2">
          <button
            onClick={onMulaiChat}
            className="w-full py-3.5 rounded-2xl text-white text-sm font-semibold transition-colors"
