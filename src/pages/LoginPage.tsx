...
interface Props { onLoginSuccess: () => Promise<void>; }
...
const STEPS = [
  { step: "01", title: "Login",          emoji: "🔐", desc: "Masukkan NIM dan password Anda untuk mengakses layanan konseling akademik dan karier." },
...
export default function LoginPage({ onLoginSuccess }: Props) {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || "NIM atau Password salah.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="auth-page">
      ...
      <div className="auth-hero">
        ...
        <div className="auth-hero__content auth-hero__content--login">
          {/* Kiri */}
          <div>
          ...
          <div className="auth-card">
            ...
            <form onSubmit={handleLogin} className="auth-form">
              <div>
                <label className="auth-form__label">NIM</label>
                <Input className="auth-form__input" placeholder="Masukkan NIM kamu" value={nim}
              ...
              <button type="submit" disabled={loading} className="auth-form__btn">
                {loading ? <><Loader2 style={{ width: 18, height: 18, animation: "spin 1s linear infinite" }} /> Memverifikasi...</> : "Masuk"}
              </button>
            </form>

            <p className="auth-form__footer">
              Belum punya akun?{" "}
              <Link to="/register" className="auth-form__link">Daftar di sini</Link>
            </p>
          </div>
        </div>
      </div>
